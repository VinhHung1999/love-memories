...
**Sprint:** ⚪ **Sprint 66 CLOSED 2026-04-26** — Daily Q&A end-to-end (mobile-rework + BE streak fix VN tz + qna noti icon). 14/14 cards · 31pt · merged sprint_66 → main. Build 114 ad-hoc final. Push noti E2E confirmed live (DB save + APNs out-app + deep-link `/daily-questions`). **Tech debt carry Sprint 67+:** (a) Input.tsx height jitter — Boss takeover, 5 attempts T440-T444 chưa kết case, Boss tự fix tiếp. (b) Vibes BE schema idle (table `daily_vibes` chỉ trên dev, prod migration NOT applied — Boss directive keep idle). (c) deploy CLI vẫn clobber dev BE .env mỗi `deploy up` (B-deploy-mobile-rework-wrapper).
